...
| 🏭 **STEP厳密生成** | ✅ | CadQuery(OpenCascade)で正確なソリッド→STEP。画像トレース形状も対応（鋳造原型直結） |
| 💠 **画像から穴・石を認識** | ✅ | 内部穴/くり抜きの検出、色付き石の色・位置・サイズ推定 → 3Dに反映 |
| ✍️ **刻印(エングレービング)** | ✅ | 文字を凹/凸で3D表示、フォント・位置・日本語対応。チャットからも「"LOVE"と刻んで」 |
| 🧪 **メッシュ水密判定** | ✅ | 生成メッシュの位相を実測しmanifold(水密)を判定。独立ソリッド数・三角形数を表示 |
| 🖼️ **PNG書き出し** | ✅ | 3DビューをそのままPNG保存（共有・提案・SNS用） |
...
- **Step 3 ✅ 本物に近づける**
  - [x] Ring / Pendant パラメトリック生成（号数・プロファイル・トップ・石 / 外形・吊り穴・バチカン）
  - [x] 数値編集の即時3D反映・製造チェック・重量/原価推定
  - [x] STL/GLB/OBJ/SVG/JSON/**PNG** 出力
  - [x] **STEP厳密生成（CadQuery）** — 画像トレース形状も含む（工場直結）
  - [x] 画像→**輪郭抽出**→3D化（背景除去/モルフォロジー/Moore追跡/RDP/対称補正）
  - [x] 画像から **内部穴・くり抜き検出**、**石検出（色/位置/サイズ）**
  - [x] **刻印(エングレービング)** 3D表示＋エディタ（凹/凸・日本語対応）
  - [x] **メッシュ水密(manifold)の実測判定**、原価の現実化（地金+工賃+石）、鋳造メモ
  - [x] AIチャット語彙の大幅拡張（宝石名/色/カット/素材/形状/数値/刻印/複合指示）
  - [x] かんたん/プロ モード・おまかせ生成・ウェルカムガイド・自動保存・エラーバウンダリ
  - [ ] さらなる部品認識（backend: segmentation）、チャットの **LLM(function-calling)** 化
- **詳細な仕組みは → [ARCHITECTURE.md](./ARCHITECTURE.md)**（Atelier↔CadQueryの関係・データフロー図解）
