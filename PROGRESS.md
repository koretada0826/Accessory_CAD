...
- 状態: ①表面ディテール+②STEP高品質 完了。本番ビルド成功、vitest 23件合格。
- iter(Q8): 製品写真風ライティング。環境マップ刷新(res1024: 大ソフトボックス＋左右フィル＋赤道ハイライト帯＋鋭い縦筋, 暗背景で高コントラスト)、露出1.15・環境主導、研磨金属roughness0.32倍(≒鏡面)/envMapIntensity3.2/clearcoat0.7、反射フロア(MeshReflectorMaterial, 既定グリッドOFF)、後処理@react-three/postprocessing導入(Bloom＋Vignette, ワイヤー時無効)。本番ビルド成功、23テスト合格。
- ユーザー反応: Q7時点で「金リングまだチープ」→Q8で写真風に。さらに「画像と同じ再現度の輝き」を要求中。

## フェーズ4: ブランド級フルリニューアル（ユーザー要望「高級ジュエリーブランド×未来的×TikTok映え」）
- iter(R1): UI全面リニューアル。dark luxuryトークン(青み深黒/上品シャンパンゴールド/プラチナブルー)、
  タイポ(Inter＋ディスプレイセリフCormorant Garamondをnext/font)、globals(シネマ暗背景/ガラスパネル/ヘアライン/箔テキスト/fade-up)、
  全パネル(TopBar/左/編集/製造/AIチャット/Welcome)を絵文字廃止→線画SVG＋ガラス＋ゴールド発色＋余白整理、
  Viewerにシネマビネット＋Atelier透かし。本番ビルド成功。
- iter(R2): アクセサリーを“欲しくなる”シグネチャーに刷新。初期表示=signatureHero(WGソリティア:大粒ダイヤ＋ミル打ち＋
  ギャラリー＋メレ)。テンプレを欲しくなる順のシグネチャー中心に再構成(ソリティア/モードシグネット/オニキスヘキサ/
  ミルグレインバンド/モダンイヤーカフ/バゲットドロップ/エメラルド/三石/コイン/フープ/パール/ドッグタグ)。リアル宝石色＋素材吟味。
  全テンプレ生成テスト追加。本番ビルド成功、25テスト合格。
- iter(R3): パヴェ/エタニティ。primitives.makePave(弧に小粒石＋両縁ミル打ちレール, テーブルを半径外へ向ける)、makeMilgrainを弧対応。
  RingParams.pave('none'|'shoulder'|'full')+paveColor。看板=パヴェ・ソリティア(中央ダイヤ＋肩パヴェ)、エタニティ全周テンプレ追加。
- iter(R4): 実写レンダリング。実写HDRIスタジオ(@pmndrs/assets/hdri/studio.exrをローカル同梱・CDN非依存)＋スタジオ3灯、
  研磨roughness0.09〜(白飛び/安テカり回避)envMapIntensity1.7露出1.0、マクロ構図(fov30・近距離)＋被写界深度DepthOfField。
- iter(R5): UI引き算で主役最大化。製造チェックを折りたたみ式(既定=細いサマリーバーのみ)→3D面積拡大。下部を可変高に。
- 状態: 造形・レンダリング・主役演出の3点を刷新。本番ビルド成功、26テスト合格。
- 次の候補: 異方性ヘアライン、金カラーの分光補正、オンボーディングのモーション、レスポンシブ詰め、メレ/ギャラリー/パヴェのSTEP反映。

## フェーズ5: 実写ジュエリー広告レベルへ（ユーザー「もっと派手ではなく、もっと本物」）
- iter(R6): ダイヤ光学＋白飛び抑制。石マテリアルからiridescence/clearcoat(発光・白濁の元)を撤去、ior2.42＋dispersion(ファイア)＋
  attenuation(吸収=透明感)で屈折・分散で輝く本物のダイヤ光学に。色明度から透過自動(明=ダイヤ/暗=オニキス艶)。envMapIntensity1.15。
  後処理を実写寄り: Bloom極控えめ(0.18/threshold0.95=石を発光させない)、露出0.92、DoFを上品に、反射床を控えめに。
- iter(R7): 爪留め・パヴェを本物構造に。makeProngs=爪先が石にかぶさるクロウ＋バスケット(根元を繋ぐ輪)。
  makePave=“光の線”脱却、1粒ずつ認識できる径/間隔＋石境界の両縁に粒留めビーズ(ビーズセッティング)。
- iter(R8): 金属の微細な反射差。sweepProfileにUV付与、手続きノイズroughnessMap(0.78〜1.0乗算)を全金属に適用し均一CG反射を解消。
...
- feat(ネックレス生成テンプレ強化): image→構造JSON→パラメトリック組立に転換。cable_chain(buildCableChain/makeOvalLink: 楕円リンク交互90°噛み合い＋引き輪＋アジャスター＋エンドタグ)、open_teardrop_frame(輪郭168/bevel8/curve128)、pave_curve/center_stone/bezel(丸ファセットN16)。analyzeImageでネックレス画像→signatureNecklaceに再構成(検出寸法反映)、失敗時も楕円板でなくオープンティアドロップへ。影2048。Viewerに編集/ヒーロー表示モード切替。29テスト合格。
...
- feat(ネックレス長さ+着用): (1)チェーンを短いオーバル→ネックライン型ループ(下=ペンダント/上に広がり首元開口/弧長等間隔191リンク)。全高83mm/aspect0.48/ペンダント24%で“首にかける自然な長さ”に。(2)着用モード追加: 表示モードに「着用」、手続きマネキン(首+肩胸の扁平ドーム/マット)をネックレス開口に配置、反射床OFF・全体構図。汎用着用ルーターの土台(現状neck対応, ring/ear等は今後)。29テスト合格。
